...
float temperature = 29.5, tdsValue = 0;
const char* ssid = "iniaaaini";
const char* password = "iniainioke";
const char* prosesFuzzy = "https://fuzzy-arina-production.up.railway.app/fuzzy";
const char* getData = "https://fuzzy-arina-production.up.railway.app/get_data";
...
int umur;
const float max_tinggi_air = 6.0;
const float min_tinggi_air = 15.0;
...
#define FIREBASE_HOST "https://kelompok-arina-default-rtdb.asia-southeast1.firebasedatabase.app"
#define FIREBASE_AUTH "AIzaSyAw8D2_YQf5sGJwhD5Y-fyg6bv680kkfZI"
...
void sendStatus() {
  if (WiFi.status() == WL_CONNECTED) { // Perbaikan: WiFi, bukan Wifi
    ...
    if (httpCode > 0) {
      ...
      StaticJsonDocument<200> response; // Perbaikan: StaticJsonDocument, bukan StatisJsonDocument
      DeserializationError error = deserializeJson(response, payload);
...
if (WiFi.status() == WL_CONNECTED) {
    ...
    http.begin(url); // Memulai koneksi ke URL
    ...
    if (httpResponseCode > 0) {
      ...
      // Parsing JSON
      StaticJsonDocument<200> doc; // Ukuran buffer tergantung pada struktur JSON
...
void kirimFirebase() {
    ...
    if (Firebase.setFloat(firebaseData, "/sensor/suhu", suhu)) {
    ...
    if (Firebase.setFloat(firebaseData, "/sensor/kelembaban", kelembaban)) {
...
void loop() {
    ...
    if (!status) {
        ...
        if (ketinggian_air > 0) {
            ...
            // Kontrol aktuator berdasarkan pembacaan sensor
            kontrolAktuator();
